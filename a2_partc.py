#    Main Author(s): Xuesong Zhou
#    Main Reviewer(s): Usiwakoti
...
#Represents an interactive button in the game.
#Attributes: x (int): The x-coordinate of the button's top-left corner.
#          y (int): The y-coordinate of the button's top-left corner.
#         width (int): The width of the button.
#         height (int): The height of the button.
#         text (str): The label displayed on the button.
#         color (tuple): The RGB color of the button's background.
#         text_color (tuple): The RGB color of the button's text.
#         outline_color (tuple): The RGB color of the button's outline.
#         outline_width (int): The thickness of the button's outline.
class Button:
...
# Represents a dropdown menu to select options like "Human" or "AI" players.
...
#         options (list): The list of selectable options.
#         current_option (int): The index of the currently selected option.

#     Methods:
#         draw(window): Renders the dropdown menu on the game window.
#         handle_event(event): Handles user interactions, such as selecting an option.
#         get_choice(): Returns the currently selected option index.
class Dropdown:
...
# Constants
# GRID_SIZE (tuple): Dimensions of the game grid (rows, columns).
# CELL_SIZE (int): Pixel size of each cell in the grid.
# WHITE (tuple): RGB color code for white.
# BLACK (tuple): RGB color code for black.
# X_OFFSET, Y_OFFSET (int): Screen offset for board placement.
# FULL_DELAY (int): Delay duration for animation steps during overflow.
GRID_SIZE = (5, 6)
CELL_SIZE = 100
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
...
reset_button = Button(900, 170, 200, 50, "Reset")      #Reset Button
status = ["", ""]
current_player = 0
...
grid_row = -1
choice = [None, None]
# Later in the event loop of your game:
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Checking for reset button clicks to reset the game board
            if event.type == pygame.MOUSEBUTTONDOWN:
                x, y = event.pos
                if reset_button.is_over((x, y)):
                    ...
                    # Handling other mouse click events, such as player moves
                    row = y - Y_OFFSET
                    col = x - X_OFFSET
    ...
    if not has_winner:
        ...
        text = bigfont.render(
            "Player " + str(winner) + " wins!", True, (0, 0, 0)
        )  # Black color
